(define (domain FOND_full_complete_5_4)
    ...
    (:action Move-away-from-pkg
        :precondition (and (not (stack-in dist-to-next-pkg)) (not (stack-in dist-to-target)))
        :effect (and (not (zero dist-to-next-pkg)) (not (zero dist-to-target)))
    )
    ...
    (:action Move-away-from-target
        :precondition (and (not (stack-in dist-to-target)))
        :effect (and (not (zero dist-to-target)))
    )
    (:action Pick-pkg
